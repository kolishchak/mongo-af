...
class Solution
  ...
  def racer_names
    @coll.find.aggregate([ {:$project =>
                               {:_id => 0, :first_name => 1, :last_name => 2}}
                         ])
  end
  ...
  def concat_names
    @coll.find.aggregate([ {:$project =>
                               {:_id => 0, :number => 1, :name => {:$concat => ["$last_name", ", ", "$first_name"]}}}
                         ])
  end
  ...
  def group_times
    @coll.find.aggregate([ {:$group =>
                               {:_id => {:age => "$group", :gender => "$gender"}, runners:{:$sum => 1}, fastet_time:{:$min => "$secs"}}}
                         ])
  end

  def group_last_names
    @coll.find.aggregate([ {:$group =>
                               {:_id => {:age => "$group", :gender => "$gender"}, last_names:{:$push => "$last_name"}}}
                         ])
  end

  def group_last_names_set
    @coll.find.aggregate([ {:$group =>
                               {:_id => {:age => "$group", :gender => "$gender"}, last_names:{:$addToSet => "$last_name"}}}
                         ])
  end
  ...
  def groups_faster_than criteria_time
    @coll.find.aggregate([ {:$group => {:_id => {:age => "$group", :gender => "$gender"}, runners:{:$sum => 1}}},
